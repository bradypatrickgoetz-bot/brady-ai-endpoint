...
${text}
    ...
    const hfResp = await fetch(hfUrl, {
      ...
      headers: {
        ...
        // If you ever add a HF token, you'd put Authorization here.
      },
      body: JSON.stringify({
        inputs: prompt,
        parameters: {
    ...
    // Try to unwrap HF-style response if it's an array with generated_text
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0 && parsed[0].generated_text) {
        ...
        // Already final JSON array
        return res.status(200).json(parsed);
      }
    } catch {
      // raw is plain text, fall through
    }
